...
4. Откройте приложение и войдите через Яндекс
5. Если вы агент, заполните `VITE_DIRECT_CLIENT_LOGIN` логином клиента
6. Проверьте `http://localhost:8787/health`
   Должен вернуться JSON с `"ok": true`
7. После входа в приложении сверху должен появиться статус `Direct connector`, а не `Demo mode`
8. Откройте раздел кампаний и агент
9. Нажмите запуск агента
10. Для проверки боевого действия найдите рекомендацию с `pause` или `resume` и примените её

Если не работает:

- `401 Missing Bearer token`:
  фронт не передал OAuth-токен
- ошибка Direct API:
