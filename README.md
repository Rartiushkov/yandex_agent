...
- `YANDEX_DIRECT_DEFAULT_MODE` - среда по умолчанию: `sandbox` или `production`
- `YANDEX_DIRECT_MASTER_TOKEN_SANDBOX` - мастер-токен для sandbox, если хотите быстро тестировать песочницу
- `YANDEX_DIRECT_MASTER_TOKEN_PRODUCTION` - мастер-токен для production, если нужен single-user режим на боевом Direct
- `YANDEX_DIRECT_OAUTH_CLIENT_ID_SANDBOX` - Client ID OAuth app для sandbox
- `YANDEX_DIRECT_OAUTH_CLIENT_SECRET_SANDBOX` - Client secret OAuth app для sandbox
- `YANDEX_DIRECT_OAUTH_CLIENT_ID_PRODUCTION` - Client ID OAuth app для боевого Direct
- `YANDEX_DIRECT_OAUTH_CLIENT_SECRET_PRODUCTION` - Client secret OAuth app для боевого Direct
- `YANDEX_DIRECT_READ_ONLY` - по умолчанию `true`, агент читает реальные данные, но не меняет боевые кампании

## Контракт backend-коннектора

Если `VITE_DIRECT_API_BASE_URL` задан, фронтенд ожидает:

- `GET /auth/direct/url?mode=sandbox|production` - вернуть ссылку на получение verification code для выбранной среды
- `GET /auth/direct/status` - вернуть отдельный статус подключения для `sandbox` и `production`
- `POST /auth/direct/exchange` - обменять verification code на Direct API token для выбранной среды
- `POST /auth/direct/logout` - сбросить одну Direct-среду или всю Direct-сессию
- `POST /auth/direct/active-mode` - сделать активной `sandbox` или `production`
...
- безопасный read-only режим на боевых данных
- multi-user подключение Direct API через verification code и серверную cookie-сессию

## Что важно для защиты задания

Сейчас агент работает на реальных данных Яндекс.Директа, но в режиме `read-only`:

- он тянет реальные кампании и статистику
- строит реальные рекомендации
- не вносит автоматические изменения в боевую рекламу

Это снижает риск и хорошо подходит для демонстрации, ревью и защиты проекта.
...
2. Пользователь отдельно выбирает `Sandbox` или `Real Direct`.
3. Пользователь нажимает `Подключить` для нужной среды.
4. Backend открывает ссылку нужного Direct OAuth app.
5. Яндекс показывает `verification code`.
6. Пользователь вставляет этот код в приложение.
7. Backend меняет код на персональный Direct API token и хранит его в серверной сессии отдельно для `sandbox` и отдельно для `production`.

Это даёт каждому клиенту подключать именно свой Яндекс.Директ, а не общий мастер-токен, и прозрачно переключаться между тестовой и боевой средой.
